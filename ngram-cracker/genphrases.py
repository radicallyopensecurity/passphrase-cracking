...
# Settings unconfigurable by command line arguments
outhistsize = 5  # Higher is slower shortphrases, but fewer duplicates (and slightly more memory)
...
'''Usage:
  ...
  genphrases.py [-v] [-b] [-n n] [-m minfreq] ngrams-file minlen maxlen
...
Arguments:
  ...
  -b
    Benchmark mode. Only output the total number of generated phrases.
...
if verbose and skippedBecauseMinfreq > 0:
    err('Skipped {} because they are <= -m\n'.format(skippedBecauseMinfreq))

for indexgram in index:
    newindex = []
    for i in reversed(index[indexgram]):
        newindex.append(i)
    index[indexgram] = newindex

if maxlen > n * 10:
    err('Warning: you requested phrases up to {} words, but supply only {}-grams. Resulting phrases are probably nonsense.\n'.format(maxlen, n))

def doShortPhrases():
    global totalPhrases

    outhist = [None for _ in range(outhistsize)]
    outhistptr = 0
    ...
    if minlen < n:
        ...
        for i in range(len(ngrams) - 1, -1, -1):
            for j in range(max(minlen, 1), min(n, maxlen + 1)):
                out = ' '.join(ngrams[i][0].split(' ')[0:j])
                if out in outhist:
                    continue
                else:
                    outhistptr = (outhistptr + 1) % outhistsize
                    outhist[outhistptr] = out
                    if not benchMode:
                        print(out)
                    else:
                        totalPhrases += 1
...
def permute(state, indexgram, length):
    ...
    if length == maxlen - 1:
        if indexgram in index:
            if benchMode:
                totalPhrases += len(index[indexgram])
            else:
        ...
        if length >= minlen:
            if not benchMode:
                ...
                totalPhrases += 1
                if totalPhrases % 10000 == 0:
                    print('Total passphrases generated: {}M'.format(totalPhrases/1000000))
...
if benchMode:
    print('Total passphrases generated: {}M'.format(totalPhrases/1000000.0))
